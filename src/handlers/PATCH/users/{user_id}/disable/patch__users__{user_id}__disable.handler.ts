 ...
 * @create date 2019-12-08 18:46:22
 * @modify date 2019-12-08 18:46:22
 * @desc Handle `PATCH /api/users/:user_id/disable`
 */
...
/**
 * Handler for handling `PATCH /api/users/:user_id/disable`
 * Disable user with _id = `:user_id`
 *
      ...
      if (error) {
        ...
        if (is_permitted) {
          ...
          if (valid_arguments_flag) {
            UserController.make_inactive(req.params.user_id, (errorUserControllermake_inactive, user = null) => {
              if (errorUserControllermake_inactive) {
                return res.status(500).json({
                  status: 'error',
                  message: errorUserControllermake_inactive,
                  user_id: req.user._id,
                  token: req.user.token
                });
              } else {
