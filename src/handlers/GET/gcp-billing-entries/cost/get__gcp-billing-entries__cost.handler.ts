 ...
 * @create date 2019-12-24 16:15:25
 * @modify date 2019-12-24 16:15:25
 * @desc Handle `GET /api/gcp-billing-entries/cost`
 */
...
/**
 * Handler for handling `GET /api/gcp-billing-entries/cost`
          ...
          if (req.query.startdate || req.query.enddate) {
            if (req.query.startdate && req.query.enddate) {
              ...
              if (startdate.isValid() && enddate.isValid() && startdate.isSameOrBefore(enddate)) {
                ...
                SELECT * FROM
                (
                  SELECT SUM(cost) AS cost_sum, currency, CAST(usage_start_time AS DATE) as cost_date
                  FROM \`${gcp_config_bigquery.table_name}\`
                  GROUP BY cost_date, currency
            ...
            .then((job: JobResponse) => {
              job[0]
                ...
                .then((rows: any) => {
                  return res.json({
                    status: 'success',
                    message: 'GCP Billing Entries cost fetched',
                    data: rows[0],
                    user_id: req.user._id,
                    token: req.user.token
                  });
